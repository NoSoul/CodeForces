    ...
    while(Q--) {
        ...
        if(Mat[x][y] == 0) {
            if(!Front(x, y) && !Back(x, y)) {
                ...
                RBTreeDel(&Tree, GetBack(x, y));
                RBTreeAdd(&Tree, GetBack(x, y) + 1);
            } else if(Front(x, y) && !Back(x, y)) {
                RBTreeDel(&Tree, GetFront(x, y));
                RBTreeAdd(&Tree, GetFront(x, y) + 1);
            } else {
                RBTreeDel(&Tree, GetFront(x, y));
                RBTreeDel(&Tree, GetBack(x, y));
                RBTreeAdd(&Tree, GetFront(x, y) + GetBack(x, y) + 1);
            }
        } else if(Mat[x][y] == 1) {
            if(!Front(x, y) && !Back(x, y)) {
                RBTreeDel(&Tree, 1);
            } else if(!Front(x, y) && Back(x, y)) {
                RBTreeDel(&Tree, GetBack(x, y) + 1);
                RBTreeAdd(&Tree, GetBack(x, y));
            } else if(Front(x, y) && !Back(x, y)) {
                RBTreeDel(&Tree, GetFront(x, y) + 1);
                RBTreeAdd(&Tree, GetFront(x, y));
            } else {
                RBTreeDel(&Tree, GetFront(x, y) + GetBack(x, y) + 1);
                RBTreeAdd(&Tree, GetFront(x, y));
                RBTreeAdd(&Tree, GetBack(x, y));
            }
        }
        Mat[x][y] ^= 1;
        struct RBNode_t *node = RBTreeMaximum(Tree.mTreeRoot, &Tree.mSentinelLeaf);
