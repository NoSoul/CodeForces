...
int main()
{
    ...
    for(i = 0; i < m; ++i) {
        ...
        RBTreeInsert(&Tree, node);
        struct RBNode_t *p = RBTreePredecessor(node, &Tree.mSentinelLeaf);
        struct RBNode_t *s = RBTreeSuccessor(node, &Tree.mSentinelLeaf);
        curNum -= (s->mKey.mPrimaryVal - p->mKey.mPrimaryVal) / (a + 1);
        curNum += (s->mKey.mPrimaryVal - x) / (a + 1) + (x - p->mKey.mPrimaryVal) / (a + 1);
        if(curNum < k) {
            printf("%d\n", i + 1);
            return 0;
        }
